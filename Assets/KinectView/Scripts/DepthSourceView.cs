...
public class DepthSourceView : MonoBehaviour
{
    ...
    {
        _Mesh = new Mesh();  //new mesh
        GetComponent<MeshFilter>().mesh = _Mesh;  //gets the mesh filter component

        _Vertices = new Vector3[width * height];  //create empty verticies vector
        _UV = new Vector2[width * height];  //create empty UV vector
        _Triangles = new int[6 * ((width - 1) * (height - 1))];  //create empty triangles vector
